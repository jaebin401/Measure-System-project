...
// ==================== 사용자 설정 ==========================
const int   PHOTO_PIN      = 2;          // BUP-50S 검정선 → D2

// 플래그 폭 [m] (M5 나사 지름 ≈ 5mm)  <- 지금은 속도 안 쓰지만, 원래 설계 흔적이라 남겨둠
...
void setup() {
  ...
  Serial.println(F("#hit\tT[s]\ttau[ms]\tI[kg*m^2]"));
}
...
void loop() {
  ...
  // ★ 진동이 멈췄는지(5초 이상 히트 없음) 체크 후, 한 번만 SUMMARY 출력
